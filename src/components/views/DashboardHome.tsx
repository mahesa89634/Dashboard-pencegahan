...
export default function DashboardHome({
  ...
  // Subdistricts distribution count
  const subdistricts = ['Rasanae Barat', 'Rasanae Timur', 'Mpunda', 'Raba', 'Asakota'];
  const subdistrictCounts = subdistricts.map(sub => ({
    name: sub,
    count: volunteers.filter(v => v.subdistrict === sub).length
  }));
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <GraduationCap className="w-4 h-4" /> Uji Kuis Aparatur
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
            ...
            <div className="space-y-3">
              {subdistrictCounts.map((sub) => {
                const percentage = volunteers.length > 0 ? Math.round((sub.count / volunteers.length) * 100) : 0;
                return (
                  <div key={sub.name} className="space-y-1">
                    <div className="flex justify-between text-xs font-semibold">
                      <span className="text-slate-700">{sub.name}</span>
                      <span className="text-slate-900 font-mono font-bold">{sub.count} Orang ({percentage}%)</span>
                    </div>
            ...
            {socializations.length > 0 && (
              <div className="bg-white/10 rounded-xl p-3 border border-white/10 space-y-2">
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-300 pt-1">
                  <span className="flex items-center gap-1">
                    <Calendar className="w-3 h-3" /> {socializations[0].date}
                  </span>
